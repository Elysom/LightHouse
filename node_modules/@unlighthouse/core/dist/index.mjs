...
import { isRelative, $URL, hasProtocol, withBase, withLeadingSlash, withoutTrailingSlash, withQuery, withTrailingSlash, normalizeURL, joinURL, withoutLeadingSlash } from 'ufo';
...
async function setupPage(page) {
  ...
  browser.on("targetchanged", async (target) => {
    ...
    if (page2) {
      if (resolvedConfig.cookies)
        await page2.setCookie(...resolvedConfig.cookies.map((cookie) => ({ domain: resolvedConfig.site, ...cookie })));
      if (resolvedConfig.extraHeaders)
...
const runLighthouseTask = async (props) => {
  ...
  const args = [
    ...
    `--routeReport=${JSON.stringify(pick$1(clonedRouteReport, ["route.url", "artifactPath"]))}`,
...
async function createUnlighthouseWorker(tasks) {
  ...
  const queueRoutes = (routes) => {
    ...
    const sortedRoutes = sortBy$1(routes, (r) => {
...
function mergeRobotsTxtConfig(config, { groups, sitemaps }) {
  const normalisedGroups = groups.filter((group) => group.userAgent.includes("*")).map((group) => {
    for (const k of ["disallow", "allow"]) {
      group[k] = group[k].filter((path) => path !== "/" && path).map((path) => {
        if (path.includes("*"))
          path = path.replace(/\*/g, ".*");
        else
          path = `${path}.*`;
  ...
  config.scanner.exclude = [.../* @__PURE__ */ new Set([
  ...
  if (!config.scanner.include.length && robotsAllows.length) {
    config.scanner.include = [.../* @__PURE__ */ new Set([
      "/*",
